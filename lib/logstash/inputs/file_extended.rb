...
require "fileutils"
require_relative "file_extended/patch"
...
class LogStash::Inputs::FileExtended < LogStash::Inputs::Base
  ...
  # The HTTP authentication endpoint which needs to be polled in order to validate the current file.
  # [Ex: http://localhost:9090/myapp/validate]. The MD5 hash of the new file is sent to this endpoint as a query
  # parameter for validation. [Ex: http://localhost:9090/myapp/validate?md5=be942917e33232e20648e1fc3cd786a3]. The
  # call is in the form of GET call. Thus, the endpoint needs to be designed accordingly.
  config :auth_endpoint, :validate => :string, :required => true

  # Any additional parameters that needs to be sent to the authentication endpoint above can be set via this config.
  # Its expected in the form of an array of Strings. If any string in the array is in the form "<NAME>=<VALUE>", then
  # this parameter will be sent as a query string to the endpoint. For any other string, it will be appended to the
  # endpoint URL above as path parameters in the order that it appears in the params array.
  # Ex: auth_params => ["shabir", "age=21", "mcgill", "average=3.85"] will result in the following URL:
  # http://localhost:9090/myapp/validate/shabir/mcgill?md5=be942917e33232e20648e1fc3cd786a3&age=21&average=3.85
  config :auth_params, :validate => :array, :required => false
  ...
  def register
    ...
    if File.directory?(@sincedb_path)
      raise ArgumentError.new("The \"sincedb_path\" argument must point to a file, received a directory:
\"#{@sincedb_path}\"")
    end
...
end # class LogStash::Inputs::FileExtended
